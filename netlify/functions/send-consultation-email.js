...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { fullName, email, phoneNumber, investmentAmount, investmentTimeline } = JSON.parse(event.body);

    // Validation
    const errors = [];
    if (!fullName || !sanitizeInput(fullName)) errors.push('Full name is required');
    ...
    if (!investmentAmount) errors.push('Investment amount selection is required');
    if (!investmentTimeline) errors.push('Investment timeline selection is required');
